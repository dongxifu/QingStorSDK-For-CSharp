...
namespace QingStorSDK.com.qingstor.sdk.utils
{
     ...
     */
        public static string getAuth(string accessKey,string secretKey,string method,string authPath,Dictionary<object, object> Params,Dictionary<object, object> headers) 
        {
            string signature = getSignature(accessKey, secretKey, method, authPath, Params, headers);
            return ("QS "+ accessKey + ":" + signature);
        }
     ...
     */
        public static string getSignature(string accessKey,string secretKey,string method,string authPath,Dictionary<object, object> Params,Dictionary<object, object> headers) 
        {
            ...
            if (Params != null) 
            {
                string[] sortedParamsKeys = new string[200];
                int keyindex2=0;
            ...
            if (canonicalized_query!=null && canonicalized_query.IndexOf("&upload_id") != -1)//?part_number=0&upload_id=1asfdsf
            {
                if (canonicalized_query.IndexOf("upload_id") > canonicalized_query.IndexOf("part_number"))
                {
                    String temp1 = canonicalized_query.Substring(canonicalized_query.IndexOf("part_number"), canonicalized_query.IndexOf("&upload_id") - canonicalized_query.IndexOf("part_number"));
                    String temp2 = canonicalized_query.Substring(canonicalized_query.IndexOf("upload_id"), canonicalized_query.Length - canonicalized_query.IndexOf("upload_id"));
                    canonicalized_query = temp2 + "&" + temp1; //singedUrl.Replace(temp1, temp2).Replace(temp2,temp1);
                }
            }
            ...
            strToSign += ("\n"+canonicalized_resource);//.Replace("?uploads??","?uploads");
