...
namespace QingStorIaasSDK.com.qingstor.sdk.utils
{
    class QSParamInvokeUtil
    {
        private static void setParameterToDictionary(MethodInfo m, object source, Dictionary<object,object> retParametersDictionary, string paramKey)
        {
            ...
            if (objValue != null) 
            {
                ...
                else if(Ts.Equals(typeof(String[])))
                {
                    ...
                    {
                        retParametersDictionary.Add(paramKey.ToString().ToLower()+"."+(i+1).ToString(), QSStringUtil.chineseCharactersEncoding(((String[])objValue)[i] + ""));
                    }
                }
                else if(Ts.Equals(typeof(string)) || Ts.Equals(typeof(string)))
                {
                    retParametersDictionary.Add(paramKey.ToString().ToLower(), QSStringUtil.chineseCharactersEncoding(objValue + ""));
                }
                else 
                {
        ...
        public static Dictionary<object,object> getRequestParams(object model, string paramType) 
        {
            ...
            {
                if (!retParametersDictionary.ContainsKey(QSConstant.HEADER_PARAM_KEY_DATE)) 
                {
                    retParametersDictionary.Add(QSConstant.HEADER_PARAM_KEY_DATE,QSSignatureUtil.formatGmtDate(DateTime.Now));
                }
